...
u8 USART3_RX_BUF[USART3_RX_LEN];      //接收缓冲,最大USART_REC_LEN个字节.
u8 USART3_TX_BUF[USART3_TX_LEN]; 

u8 UART4_RX_BUF[UART4_RX_LEN];        //接收缓冲,最大USART_REC_LEN个字节.
...
FIFO_Type Uart3_Rx_Fifo;										  //串口2环形FIFO结构变量
...
extern u8 CmdRxSemCount;     //计数型信号量
...
{   
	#if 1
		while(!USART_GetFlagStatus(DEBUG_USART,USART_FLAG_TXE));	  //USART_GetFlagStatus：得到发送状态位
...
void uart3_init(u32 bound)
{
  ...
  //串口3引脚复用映射
	...
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; //上拉
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10; //GPIOB10与GPIOB11
	...
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; //上拉
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11; //GPIOB10与GPIOB11
	GPIO_Init(GPIOB,&GPIO_InitStructure); //初始化PB10，PB11
...
void uart4_init(u32 bound){
  ...
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	//UART4端口配置
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 ; //GPIOA0与GPIOA1
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用功能
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	//速度50MHz
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; //上拉
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1; //GPIOA0与GPIOA1
	GPIO_Init(GPIOA,&GPIO_InitStructure); //初始化PA0，PA1
...
void uart6_init(u32 bound)
{
  ...
  //串口6引脚复用映射
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7; //GPIOC6与GPIOC7
	GPIO_Init(GPIOC,&GPIO_InitStructure); //初始化PC6，PC7 
...
void Uart_6_Send(const uint8_t* buf, uint16_t len)
{
	...
	for(i=0;i<200;i++)
	{
		...
		{ 
			DMA_ClearFlag(DMA2_Stream6,DMA_FLAG_TCIF6);            //清除DMA2_Steam7传输完成标
		  break;
		}
		delay_ms(10);
	}
